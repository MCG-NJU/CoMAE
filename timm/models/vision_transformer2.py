...
class Block2(nn.Module):
    ...
    def __init__(
            self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,  drop=0., attn_drop=0., init_values=True,
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        if (int(attn.size(2)) == 394):
            summ = 0
            for kk in range(0, int(attn.size(2) / 2)):
                for vv in range(int(attn.size(3) / 2), attn.size(3)):
                    value = attn[0][10][kk][vv]
                    summ = summ + value
            print(summ / (attn.size(2) / 2))
        '''
        ...
        if (int(attn.size(2)) == 394):
            summ=0
            summ2 = 0
            #for i in range(0,attn.size(0)):
            for j in range(0, attn.size(1)):
                for kk in range(0, int(attn.size(2)/2)):
                    for vv in range(int(attn.size(3)/2), attn.size(3)):
                        value = attn[0][j][kk][vv]
                        summ = summ + value

            for j in range(0, attn.size(1)):
                for kk in range(0, int(attn.size(2)/2)):
                    for vv in range(0, int(attn.size(3)/2)):
                        value = attn[0][j][kk][vv]
                        summ2 = summ2 + value

            print(summ/attn.size(1)/(attn.size(2)/2))
            print(summ2 / attn.size(1) / (attn.size(2) / 2))
        '''
...
class Block(nn.Module):
    ...
    def forward(self, x):
        pp, attn = self.attn(self.norm1(x))
        x = x + self.drop_path(pp)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, attn
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #self.hha_embedding = nn.Parameter(torch.zeros(1, 1, embed_dim))
        #self.rgb_embedding = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        #self.fusion_tokens = nn.Parameter(torch.zeros(1, 4, embed_dim))

        # Classifier head
        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()

        #self.head1 = nn.Linear(embed_dim, int(embed_dim/2))
        #self.head2 = nn.Linear(int(embed_dim/2), num_classes)

        #self.head2 = Mlp(in_features=embed_dim, hidden_features=256,out_features=num_classes,act_layer=nn.GELU, drop=0.1)
        ...
        #trunc_normal_(self.fusion_tokens, std=.02)
